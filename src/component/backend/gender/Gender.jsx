...
const Gender = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await api.post(`/genders`, gender)
    ...
    const deleteGender = async (id) => {
        ...
        try {
            const result = await api.delete(`/genders/${id}`)
            showSuccess(result.data.message);
            fetchDatas();
        } catch (error) {
            showError(error.response.data.message);
                    ...
                    <div className="glass-card create-admin-card">
                        ...
                        <form onSubmit={handleSubmit}  >
                            <div className="form-group">
                                <label className="form-label"> Gender Name</label>
                                <input type="text" name='name' value={gender?.name} onChange={handleInput} className="form-control" id="newAdminName" placeholder="e.g. Alex Rivera" />
                                ...
                                can("genders.store") && (
                                    <button type='submit' className="btn-primary" style={{ width: '100%', justifyContent: 'center' }}>
                                        {loading ? <PulseLoader
                                            ...
                                            size={12}
                                        /> : ''}
                                        <i className="bi bi-person-plus-fill" /> Create Gender Account
                                    </button>
                                )
                            }
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div className="table-actions">
                                                            ...
                                                            {
                                                                can("genders.destroy") && (
                                                                    <button className="btn-danger-sm" onClick={() => deleteGender(gender.id)} title="Delete"><i className="bi bi-trash3" /></button>
                                                                )
                                                            }
                                                        </div>
                                                    </td>
